...
{
    /**
     * return the value of role names handed in as string
     * example: 'Admin'
     * 
    ...
    public static function getRoleValue($role_name)
    {
        ...
        $sql = "SELECT role_value FROM role 
    ...
    /**
     * return the value of a status name handed in a string
     * example: 'Active'
    ...
    {
        $connection = \Yii::$app->db; 
        $sql = "SELECT status_value FROM status 
                WHERE status_name=:status_name"; 
        
        $command = $connection->createCommand($sql);
        $command->bindValue(":status_name", $status_name); $result = $command->queryOne();
        
        return $result['status_value'];
    }
    
    /**
     * returns values of user_type_name so that you can
